...
{
	/* TODO */
	term_t remainder_t, divisor_t, dividend_t, quotient_t, dummy;
	*quotient=polynomial_alloc();
	*remainder=polynomial_alloc();
	...
	do{
		...
		if(quotient_t.expo<0){
			*remainder=linkedlist_clone(dividend);
			break;
		}
		linkedlist_insert_last(*quotient, &quotient_t);

		while(linkedlist_length(*remainder)>0){
			linkedlist_remove_first(*remainder, &dummy);
		}
		...
		*remainder=linkedlist_clone(dividend);
		...
		if(linkedlist_length(*remainder)==0){
			break;
		}
